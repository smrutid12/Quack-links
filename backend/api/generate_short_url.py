...
generate_quack_url = Namespace(
    "Generate Quack Link",
    description="API for creating and fetching shortened URLs"
)

QUACK_LINK_URL = os.getenv("QUACK_LINK_URL", "http://localhost:3000")
...
original_url_response_model = generate_quack_url.model("OriginalURLResponseModel", {
...
def generate_short_url(original_url):
    ...
    short_url = f"{QUACK_LINK_URL.rstrip('/')}/{short_url_id}"
...
class FetchQuackLink(Resource):
    ...
    def get(self, short_url_id):
        ...
        Useful if someone opens backend URL directly.
        """
...
generate_quack_url.add_resource(GetOriginalURL, "/original/<string:short_url_id>")
